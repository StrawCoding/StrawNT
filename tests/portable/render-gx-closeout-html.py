#!/usr/bin/env python3
"""Render Game Compat gx5 closeout markdown to HTML (hermes-deliver)."""
...
def main() -> int:
    md_path = CLOSEOUT_DIR / "gx-closeout-report.md"
    if not md_path.is_file():
        print(f"missing {md_path}", file=sys.stderr)
        return 1
    body = md_to_html(md_path.read_text(encoding="utf-8"))
    HTML_DIR.mkdir(parents=True, exist_ok=True)
    out_path = HTML_DIR / "gx-closeout-report.html"
    doc = f"""<!DOCTYPE html>
<html lang="zh-Hant">
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1"/>
  <title>StrawWU Portable Game Compat Closeout</title>
  <style>{CSS}</style>
</head>
<body>
  <div class="wrap">
    <header>
      ...
      <h1>Game Compat Closeout</h1>
      <p class="meta">版本 {html.escape(VERSION)} · gx5-closeout · hermes-deliver</p>
    </header>
    {body}
    <footer>StrawWU Portable · native PE / strawwu-nt · game-compat · generated from gx-closeout-report.md</footer>
  </div>
</body>
</html>
"""
