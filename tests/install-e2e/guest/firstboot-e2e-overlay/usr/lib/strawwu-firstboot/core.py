...
def emit_serial_marker(marker: str = E2E_MARKER) -> None:
    """Write E2E marker to serial console and kernel log (QEMU install-firstboot E2E)."""
    line = f"{marker}\n"
    for dev in ("/dev/ttyS0", "/dev/kmsg", "/dev/console"):
        try:
...
def run_e2e() -> int:
    """Headless firstboot for QEMU install E2E — real state writes, no GTK."""
    os.environ["STRAWWU_FIRSTBOOT_FORCE"] = "1"
    log_event("info", "e2e start")
    if not set_lifecycle("running"):
        emit_serial_marker("FIRSTBOOT_FAIL")
        return 1
    steps = step_ids()
    prefs = default_prefs()
    prefs["completed_steps"] = steps
    apply_locale(str(prefs["locale"]))
    rc = complete_firstboot(prefs)
    if rc == 0:
        emit_serial_marker(E2E_MARKER)
        log_event("info", "e2e complete", marker=E2E_MARKER, steps=steps)
    else:
        emit_serial_marker("FIRSTBOOT_FAIL")
        log_event("error", "e2e failed", code=ERROR_STATE)
    return rc
