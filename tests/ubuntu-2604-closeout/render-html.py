#!/usr/bin/env python3
"""Render StrawWU Ubuntu 26.04 migration closeout markdown to HTML (hermes-deliver)."""
...
<head>
  ...
  <title>StrawWU Ubuntu 26.04 Migration Closeout</title>
  <style>{CSS}</style>
</head>
<body>
  <div class="wrap">
    <header>
      <div class="brand">StrawWU Ubuntu 26.04 Migration</div>
      <h1>Resolute 遷移 — 全 7 段 PASS 驗收</h1>
      <p class="meta">版本 {html.escape(VERSION)} · u26-m7-closeout · hermes-deliver</p>
    </header>
    <main>
{body}
    </main>
    <footer>
      StrawWU v0.6 Resolute · 7 migration stages · Post-MVP → post-d1-strawwu-drivers
    </footer>
  </div>
</body>
</html>
...
def main() -> int:
    md_path = CLOSEOUT_DIR / "ubuntu-2604-dod.md"
    if not md_path.is_file():
        print(f"FAIL: missing {md_path}", file=sys.stderr)
        return 1
    HTML_DIR.mkdir(parents=True, exist_ok=True)
    out_path = HTML_DIR / "ubuntu-2604-closeout-report.html"
