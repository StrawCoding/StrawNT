#!/usr/bin/env python3
"""Render StrawWU handbook markdown to self-contained HTML (hermes-deliver)."""
...
def render_volume(md_path: Path, title_suffix: str) -> str:
...
<body>
  <div class="wrap">
    <header>
      <div class="brand">StrawWU Handbook</div>
      <h1>{html.escape(title_suffix)}</h1>
      <p class="meta">版本 {html.escape(VERSION)} · W8-DOC · hermes-deliver</p>
    </header>
    <main>
{body}
    </main>
    <footer>
      StrawWU v0.5 完整手冊 · 支援渠道 TBD · 問題回報：strawwu-bug-report-gtk
    </footer>
  </div>
</body>
</html>
...
VOLUMES = [
    ("user-handbook.md", "user-handbook.html", "使用者手冊"),
...
def main() -> int:
    ...
    for md_name, html_name, title in VOLUMES:
        md_path = HANDBOOK / md_name
        if not md_path.is_file():
            print(f"FAIL: missing {md_path}", file=sys.stderr)
            return 1
        out_path = HTML_DIR / html_name
        out_path.write_text(render_volume(md_path, title), encoding="utf-8")
