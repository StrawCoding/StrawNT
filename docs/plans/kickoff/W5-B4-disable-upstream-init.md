# Wave 5 B4 — disable 上游初始化

| 任務 ID | w5-b4-disable-upstream-init |
|---------|-------|
| Wave | 5 |
...
cloud-init/gnome-initial-setup off
...
make test-upstream-init-disabled
make preflight
# exit 0
```
...
1. 本階段實作產物（見計畫文件）
2. `docs/plans/stage-reports/W5-B4-disable-upstream-init-report.md`
...
Hermes mark PASS → 自動啟動 **w5-w4-wincompat-gui**（勿問使用者）。
