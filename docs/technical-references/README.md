...
本目錄收錄 **Ubuntu noble (24.04) 上游權威文件**，供 StrawWU v3.0-cleanroom 各 Phase **規劃對照**與**驗收稽核**使用。

> 上游原始碼體積大（~370MB），`upstream/` 已列入 `.gitignore`。索引與更新腳本會進版控；首次使用或升級 noble 套件後請執行 `scripts/refresh-technical-references.sh`。
...
| `scripts/refresh-technical-references.sh` | 一鍵更新腳本 |

## 已收錄上游（noble）

| 領域 | 套件/來源 | StrawWU 用途 |
|------|-----------|--------------|
| Live 開機 | `casper` 1.498 | `/casper` squashfs、overlay、boot=casper |
| initramfs | `initramfs-tools` 0.142ubuntu25.8 | early3/main 結構、hook 順序 |
| 開機動畫 | `plymouth` 24.004.60 | initrd main 段 plymouthd、主題 |
| ISO 封裝 | `libisoburn`/xorriso 1.5.6 | El Torito、增量 repack |
| 開機載入 | `grub2` 2.12 | BIOS/UEFI、live 參數 |
| 安裝器 | `calamares` 3.3.5 + `calamares-settings-ubuntu` 24.04.40 | Phase 3 上游對齊 |
| Kernel | `linux-stable` v6.8.12 Documentation | kbuild、overlayfs、iso9660 |
...
2. **禁止偏離上游 Calamares 行為**（見 `~/.hermes/skills/.../calamares-sop.md`）；對照 `calamares-settings-ubuntu-24.04.40/common/`。
