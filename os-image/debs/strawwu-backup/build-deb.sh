#!/usr/bin/env bash
# build-deb.sh — Build strawwu-backup .deb (system backup / timeshift PoC).
set -euo pipefail
...
echo "=== Building strawwu-backup ${VERSION} ==="

cp -a "${SCRIPT_DIR}/usr" "${PKG_DIR}/"
chmod 755 "${PKG_DIR}/usr/bin/strawwu-backup"
...
mkdir -p "${PKG_DIR}/usr/share/doc/strawwu-backup"
cat > "${PKG_DIR}/usr/share/doc/strawwu-backup/copyright" <<'EOF'
Format: https://www.debian.org/doc/packaging-manuals/copyright-format/1.0/
Upstream-Name: strawwu-backup
...
DEB_FILE="${OUTPUT_DIR}/strawwu-backup_${VERSION}_all.deb"
