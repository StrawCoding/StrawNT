#!/usr/bin/env bash
# build-deb.sh — Build strawwu-upgrade .deb (upgrade + snapshot rollback CLI).
set -euo pipefail
...
echo "=== Building strawwu-upgrade ${VERSION} ==="

cp -a "${SCRIPT_DIR}/usr" "${PKG_DIR}/"
chmod 755 "${PKG_DIR}/usr/bin/strawwu-upgrade"
...
mkdir -p "${PKG_DIR}/usr/share/doc/strawwu-upgrade"
cat > "${PKG_DIR}/usr/share/doc/strawwu-upgrade/copyright" <<'EOF'
Format: https://www.debian.org/doc/packaging-manuals/copyright-format/1.0/
Upstream-Name: strawwu-upgrade
...
DEB_FILE="${OUTPUT_DIR}/strawwu-upgrade_${VERSION}_all.deb"
